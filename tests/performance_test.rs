use my_database_manager::{
    ...
    insert, find_all, find_paginated,
};
...
async fn test_orm_performance() {
    ...
    for i in 1..=total_records {
        let level = if i % 10 == 0 { "ERROR" } else if i % 5 == 0 { "WARN" } else { "INFO" };
        let log = PerfLog {
            id: 0,
    ...
    println!("1. Waktu Insert {} baris : {:?}", total_records, duration_insert);
    ...
    let errors = find_all::<PerfLog>(&pool, "sqlite", Some(&filter_exact)).await.unwrap();
    let duration_filter_exact = start_filter_exact.elapsed();
    assert_eq!(errors.len(), total_records / 10);
    println!("4. Waktu Filter Exact (level='ERROR') : {:?}", duration_filter_exact);
    ...
    let search_res = find_all::<PerfLog>(&pool, "sqlite", Some(&filter_like)).await.unwrap();
    let duration_filter_like = start_filter_like.elapsed();
    assert!(!search_res.is_empty());
    println!("5. Waktu Filter Like (message='5000') : {:?}", duration_filter_like);
